...
int main(int argc, char*argv[]){
    ...
    if(argc != 3){
        cout << "Error input parameter" << endl;
        return -1;
    }
    string mess = argv[1];
    string key = argv[2];
    string decrypted_mess = "";

    Word Key[Nk] = {};
    StringToWords(key, Key, Nk);

    int mLen = mess.size();
    int rounds = mLen / (Nk*32);

    for(int r=0; r<rounds; r++){
        Word CipherText[Nk] = {};
        Word Messages[Nk] = {};
        string p_mess = "";
        string mess_target = mess.substr(r*128, (r+1)*128);
        StringToWords(mess_target, CipherText, Nk);

        Word Keywords[Nr] = {};
        KeyExpansion(Key, Keywords); // Generate the round keys
        Decryption(CipherText, Keywords, Messages);
        p_mess = WordsToString(Messages, Nk);
        decrypted_mess.append(p_mess);
    }
    ofstream outfile;
    outfile.open("./info/dec.txt");
    outfile << decrypted_mess;
